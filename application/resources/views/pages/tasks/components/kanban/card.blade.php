        ...
        <div class="kanban-card-content-comntainer">
                ...
                <div class="dropdown-menu dropdown-menu-small dropdown-menu-right js-stop-propagation"
                    ...
                    @if (auth()->user()->is_team && $task->permission_edit_task)
                        <a class="dropdown-item edit-add-modal-button js-ajax-ux-request reset-target-modal-form"
                            ...
                            data-action-url="{{ urlResource('/tasks/' . $task->task_id . '/clone') }}"
                            data-modal-size="modal-lg" data-loading-target="commonModalBody" data-action-method="POST"
                            aria-expanded="false">
            ...
            <div class="x-meta">
                ...
                @if (config('system.settings_tasks_kanban_priority') == 'show')
                    <label class="label label-{{ $task->taskpriority_color }} p-t-3 p-b-3 p-l-8 p-r-8"
                        data-toggle="tooltip" title="@lang('lang.priority')">{{ $task->taskpriority_title }}</label>
                @endif
                <!--tags-->
                @if (config('system.settings_tasks_kanban_tags') == 'show')
                    <div class="wrap-words">
                        @foreach ($task->tags as $task_tag)
                            <label
                                class="label label-light-primary p-t-3 p-b-3 p-r-8 p-l-8">{{ $task_tag->tag_title }}</label>
                        @endforeach
                    </div>
                @endif
                <!--project-->
                @if (config('system.settings_tasks_kanban_project_title') == 'show')
                    <span
                        title="{{ $task->project_title ?? '---' }}"><strong>{{ cleanLang(__('lang.project')) }}:</strong>
                        {{ str_limit($task->project_title ?? '---', 68) }}</span>
                @endif
                <!--client-->
                @if (config('system.settings_tasks_kanban_client_name') == 'show')
                    <span
                        title="{{ $task->client_company_name ?? '---' }}"><strong>{{ cleanLang(__('lang.client')) }}:</strong>
                        {{ str_limit($task->client_company_name ?? '---', 68) }}</span>
                @endif
                <!--date created-->
                @if (config('system.settings_tasks_kanban_date_created') == 'show')
                    <span><strong>{{ cleanLang(__('lang.created')) }}:</strong>
                        {{ runtimeDate($task->task_created) }}</span>
                @endif
                <!--start date-->
                ...
                @if (config('system.settings_tasks_kanban_date_due') == 'show')
                    <span><strong>{{ cleanLang(__('lang.due')) }}:</strong>:
                        {{ runtimeDate($task->task_date_due) }}</span>
                @endif
                ...
                <div class="col-6 x-icons">
                    ...
                    @if (runtimeArchivingOptions())
                        <span class="x-icon {{ runtimeActivateOrAchive('archived-icon', $task->task_active_state) }}"
                            id="archived_icon_{{ $task->task_id }}" data-toggle="tooltip"
                    ...
                    @if ($task->has_attachments)
                        <span class="x-icon display-inline-block vm "><i class="mdi mdi-attachment"></i>
                            @if ($task->count_unread_attachments > 0)
                                <span class="x-notification"
                                    id="card_notification_attachment_{{ $task->task_id }}"></span>
                            @endif
                        </span>
                    @endif
                    ...
                    @if ($task->has_comments)
                        <span class="x-icon display-inline-block vm p-t-3"><i
                                class="mdi mdi-comment-text-outline"></i>
                            @if ($task->count_unread_comments > 0)
                                <span class="x-notification"
                                    id="card_notification_comment_{{ $task->task_id }}"></span>
                            @endif
                        </span>
                    @endif
                ...
                <div class="col-6 x-assigned">
                    @foreach ($task->assigned as $user)
                        <img src="{{ getUsersAvatar($user->avatar_directory, $user->avatar_filename) }}"
                            data-toggle="tooltip" title="" data-placement="top" alt="{{ $user->first_name }}"
                            class="img-circle avatar-xsmall" data-original-title="{{ $user->first_name }}">
                    @endforeach
                </div>
            </div>
        </div>
