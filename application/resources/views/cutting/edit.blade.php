...
@section('content')
    <div class="container">
        <h1>Create Cutting Plan</h1>


        <form id="edit-product-form" method="POST" action="{{ route('admin.cutting.update', $plan->id) }}">
            @csrf
            @method('PUT')
                ...
                <div class="col-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="created_by" name="created_by"
                            value="{{ $cutting->created_by ?? '' }}" required>
                        @error('created_by')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select class="form-control" id="department" name="department_id" required>
                            @foreach ($departments as $department)
                                <option value="{{ $department->id }}"
                                    {{ $cutting && $cutting->department_id == $department->id ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <table class="w-full border-collapse border border-zinc-600 mb-4 table-center" id="raw-materials-table">
                ...
                <tbody>
                    @if (!empty($cutting_raw_materials))
                        @foreach ($cutting_raw_materials as $index => $rawItem)
                            <tr>
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '.raw-item-select', function() {
                ...
                if (selectedItemId) {
                    ...
                    $.ajax({
                        method: 'POST',
                        url: '{{ route('admin.cutting.get_warehouse') }}',
                        data: {
                            'id': selectedItemId
                        },
                    ...
                    $.ajax({
                        method: 'POST',
                        url: '{{ route('admin.cutting.get_unit') }}',
                        data: {
                            'id': selectedItemId
                        },
                        success: function(resp) {
