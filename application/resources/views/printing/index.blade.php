                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach ($printings as $printing)
                                <tr>
                                    <td>{{ $printing->product->name }}</td>
                                    <td>{{ $printing->created_by }}</td>
                                    <td>
                                        @if ($printing->status == 0)
                                            <button type="button" class="btn btn-success start-btn"
                                                data-id="{{ $printing->id }}" data-status="{{ $printing->status }}">
                                                Start
                                            </button>
                                        ...
                                        @elseif ($printing->status == 2)
                                            <button type="button" class="btn btn-danger start-btn" disabled>
                                                Finished
                                            </button>
                                            @if ($printing->output_name == null)
                                                <button type="button" class="btn btn-success add-output-btn"
                                                    data-id="{{ $printing->id }}"
                                                    data-output="{{ json_encode([
                                                        'output_name' => $printing->product->name,
                                                        'output_quantity' => $printing->output_quantity,
                                                        'output_actual_quantity' => $printing->output_actual_quantity,
                                                        'output_loss_quantity' => $printing->output_loss_quantity,
                                                        'output_found_quantity' => $printing->output_found_quantity,
                                                        'output_damaged_quantity' => $printing->output_damaged_quantity,
                                                    ]) }}">
                                                    Add Output
                                                </button>
                                            @endif
                                    ...
                                    <td id="start_date_{{ $printing->id }}">{{ $printing->start_date ?? 'N/A' }}</td>
                                    <td id="end_date_{{ $printing->id }}">{{ $printing->end_date ?? 'N/A' }}</td>
                                    <td>
                                        <a href="{{ route('admin.printing.show', $printing->id) }}"
                                            class="btn btn-info btn-sm">View</a>
                                        @if (empty($printing->output_name))
                                            <a href="{{ route('admin.printing.edit', $printing->plan_id) }}"
                                                class="btn btn-warning btn-sm">Edit</a>
                                            <form action="{{ route('admin.printing.destroy', $printing->id) }}"
                                        ...
                                        @endif
                                        @if (!empty($printing->output_name) && empty($printing->stiching))
                                            <a href="{{ route('admin.stiching.edit', $printing->plan_id) }}"
                                                class="btn btn-warning btn-sm">Go to Stiching</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form id="addOutputForm" method="POST">
                    @csrf
                    <input type="hidden" id="printingId" name="id">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.start-btn', function() {
                ...
                $.ajax({
                    url: '{{ route('admin.printing.start') }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: printingId
                    ...
                    success: function(response) {
                        ...
                        if (response.status === 1) {
                            ...
                            $('.add-output-btn[data-id="' + printingId + '"]').show();
                        ...
                        $('#start_date_' + printingId).text(response.start_date || 'N/A');
                        $('#end_date_' + printingId).text(response.end_date || 'N/A');
            ...
            $('#addOutputForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: '{{ route('admin.printing.output_update') }}',
                    type: 'POST',
                    data: formData,
                    success: function(response) {
                        $('#success-message').removeClass('d-none').text(response.message);
