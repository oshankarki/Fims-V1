...
@section('content')
    <div class="container">
        <h1>Create Final Plan</h1>


        <form id="edit-product-form" method="POST" action="{{ route('admin.final.update', $plan->id) }}">
            @csrf
            @method('PUT')
                ...
                <div class="col-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="created_by" name="created_by"
                            value="{{ $final->created_by ?? '' }}" required>
                        @error('created_by')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select class="form-control" id="department" name="department_id" required>
                            @foreach ($departments as $department)
                                <option value="{{ $department->id }}"
                                    {{ $final && $final->department_id == $department->id ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <h3 class="text-center">Stiching Output Details</h3>

            <div class="row">

                <div class="col-4">
                    <div class="form-group">
                        <label for="stiching_output_name">Stiching Output Name</label>
                        <input type="text" class="form-control" id="stiching_output_name" name="stiching_output_name"
                            value="{{ $stiching->output_name }}" required readonly>
                    </div>
                </div>
                <div class="col-4">
                    <div class="form-group">
                        <label for="stiching_output_name">Quantity</label>
                        <input type="text" class="form-control" id="stiching_output_name" name="stiching_output_name"
                            value="{{ $stiching->output_quantity }}" required readonly>
                    </div>
                </div>
                <div class="col-4">
                    <div class="form-group">
                        <label for="stiching_output_name">Actual Quantity</label>
                        <input type="text" class="form-control" id="stiching_output_name" name="stiching_output_name"
                            value="{{ $stiching->output_actual_quantity }}" required readonly>
                    </div>
                </div>
                <div class="col-4">
                    <div class="form-group">
                        <label for="stiching_output_name">Loss Quantity</label>
                        <input type="text" class="form-control" id="stiching_output_name" name="stiching_output_name"
                            value="{{ $stiching->output_loss_quantity }}" required readonly>
                    </div>
                </div>
                <div class="col-4">
                    <div class="form-group">
                        <label for="stiching_output_name">Found Quantity</label>
                        <input type="text" class="form-control" id="stiching_output_name" name="stiching_output_name"
                            value="{{ $stiching->output_found_quantity }}" required readonly>
                    </div>
                </div>
                <div class="col-4">
                    <div class="form-group">
                        <label for="stiching_output_name">Damaged Quantity</label>
                        <input type="text" class="form-control" id="stiching_output_name" name="stiching_output_name"
                            value="{{ $stiching->output_damaged_quantity }}" required readonly>
                    </div>
                </div>
                <div class="col-4" style="display: none;">
                    <div class="form-group">
                        <label for="stiching_id">Stiching ID</label>
                        <input type="text" class="form-control" id="stiching_id" name="stiching_id"
                            value="{{ $stiching->id }}" required readonly>
                    </div>
                </div>
            ...
            <table class="w-full border-collapse border border-zinc-600 mb-4 table-center" id="raw-materials-table">
                ...
                <tbody>
                    <tr>
                        @if (!empty($final_raw_materials))
                            @foreach ($final_raw_materials as $index => $rawItem)
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '.raw-item-select', function() {
                ...
                if (selectedItemId) {
                    ...
                    $.ajax({
                        method: 'POST',
                        url: '{{ route('admin.final.get_warehouse') }}',
                        data: {
                            'id': selectedItemId
                        },
                    ...
                    $.ajax({
                        method: 'POST',
                        url: '{{ route('admin.final.get_unit') }}',
                        data: {
                            'id': selectedItemId
                        },
                        success: function(resp) {
